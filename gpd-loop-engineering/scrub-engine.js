...
function mountScrollWorld(container, config) {
  ...
  function loadClip(s) {
    ...
    // Browsers block fetch(file://), but direct local video sources remain seekable.
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => mountVideo(URL.createObjectURL(blob)))
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // Center a true dissolve on every non-frame-locked boundary. At a seam the
      // outgoing and incoming scenes each sit at 50% instead of swapping layers.
      let op = 1;
      if (i > 0) op *= smooth((y - (s.start - fade)) / (2 * fade));
      if (i < NSEG - 1) op *= smooth(((s.end + fade) - y) / (2 * fade));
